...
class StateCityController extends Controller
{
    ...
    public function __construct(States $state, Cities $citie)
    {
        $this->state = $state;
        $this->citie = $citie;
    }
    public function savestatecity(Request $request){
$states = array(
...
for($i = 0; $i < count($states); $i++) {
    ...
    $save = $this->state::create($state_data);
    foreach($states[$keys[$i]] as $cities) {
    	foreach($cities as $cities_value) {
             $city_data = [
            'state_id' => $save->id,
            'city' => $cities_value,
        ];    
    $save_city = $this->citie::create($city_data);
        }
    }
}
}
